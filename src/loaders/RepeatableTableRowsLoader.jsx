...
const RepeatableTableRowsLoader = props => {
  const { rows = 5 } = props
  const rowHeight = 60

  return (
    <ContentLoader viewBox={`0 0 1500 ${rowHeight * rows}`} backgroundColor="#d4e0f2" foregroundColor="#a8afb8" {...props}>
...
RepeatableTableRowsLoader.metadata = {
  name: 'Lukas Werner',
  github: 'sherpaPSX',
  description:
    'Repeatable table rows. You can easily define number of rows in props and generate then in one svg',
  filename: 'RepeatableTableRowsLoader',
}

export default RepeatableTableRowsLoader
