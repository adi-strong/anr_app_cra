...
import {AppOffCanvas, APIPagination, FallBackRender} from "../../../components";
...
import AddProvincesForm from "./addProvincesForm";
import {
  nbProvincesPages,
  useGetProvincesListQuery,
  useLazyGetPaginatedProvincesListQuery, useLazyGetSearchedProvincesListQuery
} from "../model/province.api.slice";
...
export default function ProvincesList() {
  ...
  const [getPaginatedProvincesList, {
    data: paginatedItems = [],
  ...
  const [getSearchedProvincesList, {
    data: searchedItems = [],
  ...
  const {data: provinces = [], isLoading, isFetching, isError, error, refetch} = useGetProvincesListQuery(nbPages)
  ...
  const onTogglePages = async ({target}): void => {
    ...
    else if (isPaginated) await getPaginatedProvincesList({page, pages: nbPages})
    else await refetch()
  }
  ...
  const onPaginateQuery = async p => {
    try {
      setIsPaginated(true)
      setIsSearched(false)
      await getPaginatedProvincesList({page: p, pages: nbPages})
    }
  ...
  const onSearchQuery = async (e, name) => {
    ...
    try {
      ...
      setIsPaginated(false)
      setPage(1)
      await getSearchedProvincesList(name)
    }
    catch (e) { toast.error('Problème de connexion.') }
  }
  ...
  items = useMemo(() => {
    ...
    if (!(isError && isPaginatedError)) {
      ...
      else if (provinces.length > 0) obj = provinces.filter(p =>
        p.name.toLowerCase().includes(search.keyword.toLowerCase())
      )
      else obj = []
    }
  ...
  return (
    <ErrorBoundary fallbackRender={FallBackRender}>
      ...
      <AppOffCanvas
        show={show}
        onHide={toggleShow}
        title={<><i className='bi bi-plus'/> Enregistrement fiches</>}
        children={<AddProvincesForm onHide={toggleShow} pages={nbPages} onRefresh={onProvinceRefresh}/>}/>
    </ErrorBoundary>
  )
}
