...
import {useEditFolderTypeMutation} from "../model/folder.type.api.slice";
import toast from "react-hot-toast";
import {Button, Form, Spinner} from "react-bootstrap";
import {onFieldChange} from "../../../services/form.handler.service";

export default function EditFolderTypeForm({data, onRefresh, onHide}) {
  const [fields, setFields] = useState({name: ''})
  const [errors, setErrors] = useState({name: null})
  const [validate, setValidate] = useState(false)
  const [editFolderType, {isLoading, isError, error}] = useEditFolderTypeMutation()
