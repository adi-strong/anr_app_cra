...
import {APIPagination, AppOffCanvas, AuthorizedComponent, FallBackRender} from "../../../components";
...
export default function YearsList() {
  ...
  const {data: years = [], isLoading, isFetching, isError, error, refetch} = useGetYearsListQuery(nbPages)
  ...
  items = useMemo(() => {
    ...
    if (!(isError && isPaginatedError)) {
      ...
      else if (years.length > 0) obj = years.filter(p =>
        p.name.toLowerCase().includes(search.keyword.toLowerCase())
      )
      else obj = []
    }
  ...
  return (
    <ErrorBoundary fallbackRender={FallBackRender}>
          ...
          <div>
            <Table className='text-nowrap'>
              ...
              <tr>
                ...
                <th className='align-middle'>ACTIVE / INACTIVE</th>
      ...
      <AppOffCanvas
        show={show}
        onHide={toggleShow}
        title={<><i className='bi bi-plus'/> Création session</>}
        children={<YearForm onHide={toggleShow} onRefresh={onRefresh}/>}/>
    </ErrorBoundary>
  )
}
