...
import {useGetLeaveAgentsQuery} from "../model/agent.stats.api.service";
...
const LeaveAgents = () => {
  ...
  const {data: agents=[], isLoading, isFetching, isError, error, refetch} = useGetLeaveAgentsQuery('LIST')
  ...
  return (
    <ErrorBoundary fallbackRender={FallBackRender}>
      <PageHeading title='Agents en congé'/>
      <PageLayout>
        <AppBreadcrumb title='Agents en congé'/>
        ...
        <Card>
          <Card.Header className='bg-white pt-5'>
            <Card.Title>
              ...
              <i className='bi bi-joystick text-primary'/> Liste des agents en congé
            </Card.Title>
...
export default memo(LeaveAgents)
