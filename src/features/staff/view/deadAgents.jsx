...
import {useGetDeadAgentsQuery} from "../model/agent.stats.api.service";
...
const DeadAgents = () => {
  ...
  const {data: agents=[], isLoading, isFetching, isError, error, refetch} = useGetDeadAgentsQuery('LIST')
  ...
  return (
    <ErrorBoundary fallbackRender={FallBackRender}>
      <PageHeading title='Agents décédés'/>
      <PageLayout>
        <AppBreadcrumb title='Agents décédés'/>
        ...
        <Card>
          <Card.Header className='bg-white pt-5'>
            <Card.Title>
              ...
              <i className='bi bi-sign-dead-end text-primary'/> Liste des agents décédés
            </Card.Title>
...
export default memo(DeadAgents)
