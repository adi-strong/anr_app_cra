...
import {useGetActiveAgentsQuery} from "../model/agent.stats.api.service";
...
const ActiveAgents = () => {
  ...
  const {data: agents=[], isLoading, isFetching, isError, error, refetch} = useGetActiveAgentsQuery('LIST')
  ...
  return (
    <ErrorBoundary fallbackRender={FallBackRender}>
      <PageHeading title='Agents actifs'/>
      <PageLayout>
        <AppBreadcrumb title='Agents actifs'/>
        ...
        <Card>
          <Card.Header className='bg-white pt-5'>
            <Card.Title>
              ...
              <i className='bi bi-people-fill text-primary'/> Liste des agents actifs
            </Card.Title>
...
export default memo(ActiveAgents)
