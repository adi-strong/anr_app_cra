...
import {useGetUnavailableAgentsQuery} from "../model/agent.stats.api.service";
...
const UnavailableAgents = () => {
  ...
  const {data: agents=[], isLoading, isFetching, isError, error, refetch} = useGetUnavailableAgentsQuery('LIST')
  ...
  return (
    <ErrorBoundary fallbackRender={FallBackRender}>
      <PageHeading title='Agents indisponibles'/>
      <PageLayout>
        <AppBreadcrumb title='Agents indisponibles'/>
        ...
        <Card>
          <Card.Header className='bg-white pt-5'>
            <Card.Title>
              ...
              <i className='bi bi-shield-lock text-primary'/> Liste des agents indisponibles
            </Card.Title>
...
export default memo(UnavailableAgents)
