...
export default function AgentsList() {
  ...
  const {data: agents = [], isLoading, isFetching, isError, error, refetch} = useGetAgentsListQuery(nbPages)
  ...
  items = useMemo(() => {
    ...
    if (!(isError && isPaginatedError)) {
      if (page > 1 && paginatedItems.length > 0 && isPaginated) obj = paginatedItems.filter(p =>
        ...
        (p.name.toLowerCase().includes(search.keyword.toLowerCase())) ||
        (p?.lastName && p.lastName.toLowerCase().includes(search.keyword.toLowerCase())) ||
        (p?.firstName && p.firstName.toLowerCase().includes(search.keyword.toLowerCase()))
      )
      else obj = []
    }
    else obj = []
  ...
  sItems = useMemo(() => {
    ...
    if (!isSearchedError && isSearched) {
      if (searchedItems.length > 0) obj = searchedItems.filter(p =>
        (p.name.toLowerCase().includes(search.keyword.toLowerCase())) ||
        (p?.lastName.toLowerCase().includes(search.keyword.toLowerCase())) ||
        (p?.firstName.toLowerCase().includes(search.keyword.toLowerCase()))
      )
      else obj = []
    }
    else obj = []
      ...
      <Card.Header className='bg-white pt-5'>
        ...
        <Row>
          <Col md={6} className='mb-2 d-flex'>
            <Link to='/app/agents/add' className='me-1 mb-1 btn btn-primary'>
              Enregistrer
            </Link>
