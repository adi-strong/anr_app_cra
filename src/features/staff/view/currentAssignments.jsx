...
import {useGetCurrentAssignmentsQuery} from "../model/ass.stats.api.slice";
...
const CurrentAssignments = () => {
  ...
  const {data: agents=[], isLoading, isFetching, isError, error, refetch} = useGetCurrentAssignmentsQuery('LIST')
  ...
  return (
    <ErrorBoundary fallbackRender={FallBackRender}>
      <PageHeading title='Affectations en cours'/>
      <PageLayout>
        <AppBreadcrumb title='Affectations en cours'/>
        ...
        <Card>
          <Card.Header className='bg-white pt-5'>
            <Card.Title>
              ...
              <i className='bi bi-bookmark-check text-primary'/> Affectations en cours
            </Card.Title>
...
export default memo(CurrentAssignments)
