...
} from "../model/society.api.slice";
import {societyItems} from "../model/society.service";
...
export default function SocietiesList() {
  ...
  const {data: agents = [], isLoading, isFetching, isError, error, refetch} = useGetSocietiesListQuery(nbPages)
    ...
    <ErrorBoundary fallbackRender={FallBackRender}>
      <Card.Header className='bg-white pt-5'>
        ...
        <Row>
          <Col md={6} className='mb-2 d-flex'>
            <Button disabled={isLoading} onClick={toggleShow} className='me-1 mb-1'>
      ...
      <div>
        <Table className='text-nowrap'>
          ...
          <tr>
            {societyItems.length > 0 && societyItems.map(t =>
      ...
      <AppOffCanvas
        title={<><i className='bi bi-plus'/> Enregistrement société</>}
        children={<SocietyForm onHide={toggleShow} onRefresh={onRefresh}/>}
        show={show}
        onHide={toggleShow}/>
    </ErrorBoundary>
  )
