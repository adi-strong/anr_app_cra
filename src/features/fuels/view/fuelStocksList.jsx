...
import FuelStockItem from "./fuelStockItem";
import SimplePagination from "../../../components/paginations/SimplePagination";
import AddFuelStockForm from "./addFuelStockForm";

export default function FuelStocksList() {
  ...
  const {data: agents = [], isLoading, isFetching, isError, error, refetch} = useGetFuelSitesListQuery(nbPages)
    ...
    <ErrorBoundary fallbackRender={FallBackRender}>
      ...
      <Row className='mt-3'>
        <Col md={6} className='mb-2 d-flex'>
          <Button disabled={isLoading} onClick={toggleShow} className='mb-1'>
      ...
      {!isError && !isSearched && items.length > 0 && items?.map(f =>
        <FuelStockItem key={f.id} data={f} onRefresh={onRefresh} isFetching={isFetching} pages={nbPages}/>)}
      
      {isSearched && currentItems?.length > 0 && currentItems?.map(f =>
        <FuelStockItem key={f.id} data={f} onRefresh={onRefresh} isFetching={isFetching} pages={nbPages}/>)}
      
      {!isSearched && nbFuelSitesPages > 1 &&
      ...
      <AppOffCanvas
        children={<AddFuelStockForm onHide={toggleShow} onRefresh={onRefresh}/>}
        title={<><i className='bi bi-plus'/> Enregistrement site</>}
        show={show}
        onHide={toggleShow}/>
    </ErrorBoundary>
  )
