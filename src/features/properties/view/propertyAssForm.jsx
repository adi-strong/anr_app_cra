...
import {vehicleAssErrors, vehicleAssFields} from "../../vehicles/model/vehicle.service";
...
import {usePostNewPropertyAssignmentMutation} from "../model/property.ass.api.slice";

export default function PropertyAssForm({vehicle, onHide, onRefresh}) {
  ...
  const [postNewPropertyAssignment, {isLoading, isError, error}] = usePostNewPropertyAssignmentMutation()
  ...
  const onSubmit = async (e) => {
    ...
    if (vehicle) {
      formData.append('property', vehicle['@id'])
      
      if (!isSelected) {
        if (!(fields.startAt && fields.endAt)) toast.error('La durée doit être renseignée.')
        else {
          send = await postNewPropertyAssignment(formData)
          if (send?.data) {
